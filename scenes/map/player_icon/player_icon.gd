...
func _unhandled_input(event):
	if off_cooldown:
		...
		#puts player on cooldown
		time_to_next_movement = 0
		off_cooldown = false
...
func _process(delta):
	var movement_cooldown = 0.125
	
	if time_to_next_movement < movement_cooldown:
		time_to_next_movement += delta
	else:
		off_cooldown = true
