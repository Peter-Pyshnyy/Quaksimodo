...
func _on_btn_a_4_button_down():
	var input = $Answer4.text
	if input == "": return
	if question.fn_type in question.enum2str_comp.keys():
		var parser:MFunc_comp = MFunc_comp.new()
		var type = question.fn_type
		$Answer4/Label.visible = true
		$Answer4/Label.text = parser.parse_polynomial_comp(question.enum2str_comp[type], input).to_string()
	else:
		var parser:MFunc = MFunc.new()
		$Answer4/Label.visible = true
		$Answer4/Label.text = parser.parse_polynomial(input).to_string()


func _on_btn_a_4_button_up():
	$Answer4/Label.visible = false


func _on_btn_a_5_button_down():
	var input = $Answer5.text
	if input == "": return
	if question.fn_type in question.enum2str_comp.keys():
		var parser:MFunc_comp = MFunc_comp.new()
		var type = question.fn_type
		$Answer5/Label.visible = true
		$Answer5/Label.text = parser.parse_polynomial_comp(question.enum2str_comp[type], input).to_string()
	else:
		var parser:MFunc = MFunc.new()
		$Answer5/Label.visible = true
		$Answer5/Label.text = parser.parse_polynomial(input).to_string()


func _on_btn_a_5_button_up():
	$Answer5/Label.visible = false
